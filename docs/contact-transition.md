...
Reproduce the artifact and audit with `uv run tools/build_road_surface.py`. Standalone analytical and point mechanics unit tests were removed at the user's request. The results below describe historical verification, not a currently runnable unit suite.
