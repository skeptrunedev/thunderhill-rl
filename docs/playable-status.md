...
A privileged, conservative pure pursuit test driver completed a full legal lap through the actual agent API: all 32 ordered gates, no offroad ticks or crashes, 40,308 physics ticks and 335.900 seconds of simulated riding. This validates the circuit and recording loop, not model learning or real world lap performance. The trace is generated by `tools/drive_lap.py`.
...
4. Verify the exported universal Mac application on the owner’s laptop. Package structure and texture formats pass local checks, and a native Linux export runs the simulation, but these do not establish Mac runtime compatibility. Verify actual human riding, controller inputs, frame pacing and memory on the owner's laptop. Render quality and sustained performance are unproven there.
