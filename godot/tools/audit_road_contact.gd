...
func _run() -> void:
	...
	if mesh_node == null:
		push_error("No rendered road mesh found")
		quit(2)
		return
	...
	if indices.is_empty() or indices.size() % 3 != 0:
		push_error("Invalid rendered road triangle array")
		quit(2)
		return
	...
	for index in range(0, indices.size(), 3):
		...
		for weight in [
			...
			if not point.is_finite() or not normal.is_finite() or normal.length_squared() < 0.99:
				push_error("Invalid rendered road triangle")
				quit(2)
				return
			...
			if not is_finite(height) or not contact.normal.is_finite():
				push_error("Nonfinite road contact")
				quit(2)
				return
			...
			normal_errors.append(normal_angle)
			...
			rows.append(
				{
					...
					"ground_above_road_m": penetration,
	...
	var report := {
		"purpose": "Measure known road discrepancy, not physics acceptance",
		...
		"p95_normal_angle_deg": normal_errors[int(normal_errors.size() * 0.95)],
	...
	if file == null:
		push_error("Cannot write road audit")
		quit(2)
		return
	...
	file.close()
	print("ROAD_CONTACT_AUDIT ", JSON.stringify(report))
