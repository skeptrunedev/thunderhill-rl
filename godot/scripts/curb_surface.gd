...
extends RefCounted
## One triangle stream supplies curb rendering and contact. No boundary expansion.
const CELL := 16.0
...
static func _cross(a: Vector3, b: Vector3, c: Vector3) -> float:
...
func add_quad(
	...
	for p: Vector3 in points:
		if not p.is_finite():
			return "Nonfinite curb vertex"
	for uv: Vector2 in coords:
		if not uv.is_finite():
			return "Nonfinite curb UV"
	var first := _cross(a, b, c)
	var second := _cross(a, c, d)
	if first == 0.0 or second == 0.0 or signf(first) != signf(second):
		return "Degenerate or folded curb quad"
	...
	for order: Array in orders:
		...
		if maxf(absf(x0), maxf(absf(x1), maxf(absf(z0), absf(z1)))) > 2147483646.0:
			return "Curb spatial index coordinate exceeds integer range"
		var size: float = (x1 - x0 + 1.0) * (z1 - z0 + 1.0)
		if size > MAX_INDEX_CELLS_PER_QUAD - count:
			return "Curb quad exceeds spatial index budget"
		count += int(size)
...
func sample(p: Vector3) -> Dictionary:
	...
	for index: int in grid.get(Vector2i(floori(p.x / CELL), floori(p.z / CELL)), []):
		...
		if _cross(a, b, p) < 0.0 or _cross(b, c, p) < 0.0 or _cross(c, a, p) < 0.0:
			continue
		var determinant := _cross(a, b, c)
		var u := _cross(a, p, c) / determinant
		var v := _cross(a, b, p) / determinant
		...
		result = {"height": height, "normal": normal, "on_curb": true, "triangle_id": index}
	return result
