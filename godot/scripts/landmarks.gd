...
func build(track: Node3D) -> void:
	_track = track
	var data: Dictionary = JSON.parse_string(FileAccess.get_file_as_string("res://data/landmarks.json"))
...
func _commit(surface: SurfaceTool, mat: Material, label: String) -> void:
	if surface.commit_to_arrays()[Mesh.ARRAY_VERTEX].is_empty():
		return
...
func _build_building(row: Dictionary) -> void:
	...
	for i in range(0,triangles.size(),3):
		...
		_tri(_roofs,Vector3(a.x,roof_y,a.y),Vector3(b.x,roof_y,b.y),Vector3(c.x,roof_y,c.y),true)
		...
		if row.kind == "carport":
			...
			if length>5.0:
				...
				for j in count:
					...
					_quad(_glass,center-along*half+Vector3.UP*low,center+along*half+Vector3.UP*low,center+along*half+Vector3.UP*top,center-along*half+Vector3.UP*top)
					if height>6.0:
						_quad(_glass,center-along*half+Vector3.UP*4.4,center+along*half+Vector3.UP*4.4,center+along*half+Vector3.UP*5.7,center-along*half+Vector3.UP*5.7)
...
func _pavement_triangle(a: Vector2, b: Vector2, c: Vector2) -> void:
	...
	if float(lengths[edge])>64.0:
		...
		return
	_tri(_paving,_point([a.x,a.y],0.045),_point([b.x,b.y],0.045),_point([c.x,c.y],0.045),true)
...
func _build_paving(row: Dictionary) -> void:
	...
	for i in range(0,triangles.size(),3):
		_pavement_triangle(polygon[triangles[i]],polygon[triangles[i+1]],polygon[triangles[i+2]])
	...
	for i in row.points.size()-1:
		...
		for j in count:
			...
			if row.get("kind","")=="pit_wall":
				_beam(_concrete,p+Vector3.UP*height*.5,q+Vector3.UP*height*.5,0.35,height)
			else:
...
func _build_marshal(row: Dictionary) -> void:
	...
	_tri(_roofs,base+Vector3(-1.1,2.3,-1),base+Vector3(1.1,2.3,-1),base+Vector3(1.1,2.3,1),true)
	_tri(_roofs,base+Vector3(-1.1,2.3,-1),base+Vector3(1.1,2.3,1),base+Vector3(-1.1,2.3,1),true)
...
func _leaf_cluster() -> ArrayMesh:
	...
	for i in 110:
		...
		var axis := Vector3(rng.randf_range(-1,1),rng.randf_range(-0.6,0.6),rng.randf_range(-1,1)).normalized()
...
func _build_trees(rows: Array) -> void:
	...
	for i in rows.size():
		...
		for branch in 7:
			...
			var tip := base+Vector3(cos(angle)*radius*.67,height*(.67+rng.randf()*.14),sin(angle)*radius*.67)
		...
		for crown in CLUSTERS:
			...
			var center := base+Vector3(cos(angle)*spread,height*(.60+.21*(1-radial)+rng.randf_range(-.06,.09)),sin(angle)*spread)
			var scale := radius*rng.randf_range(.26,.38)
			foliage.set_instance_transform(i*CLUSTERS+crown,Transform3D(Basis(Vector3.UP,angle).scaled(Vector3(scale,scale*1.1,scale)),center))
			foliage.set_instance_color(i*CLUSTERS+crown,Color.WHITE.lerp(Color("b8b191"),rng.randf()*.3))
